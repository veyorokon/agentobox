...
interface BoxCostProps {
  hours?: number;
  minutes?: number;
}

export function BoxCost({ hours = 1, minutes = 0 }: BoxCostProps) {
  const formatDigit = (n: number) => n.toString().padStart(2, '0');
  const [h1, h2] = formatDigit(hours).split('');
  const [m1, m2] = formatDigit(minutes).split('');
      ...
      <div className="bg-amber-700/80 rounded-xl p-4 mb-3">
        <div className="flex items-center justify-center gap-1">
          ...
          <div className="flex gap-0.5">
            <DigitDisplay value={h1} />
            <DigitDisplay value={h2} />
          </div>
          ...
          {/* Minute digits */}
          <div className="flex gap-0.5">
            <DigitDisplay value={m1} />
            <DigitDisplay value={m2} />
          </div>
        </div>
      </div>

      {/* Label */}
      <p className="text-center text-sm font-semibold text-gray-800">COMPUTE TIME</p>
    </div>
  );
}
...
function DigitDisplay({ value }: { value: string }) {
  return (
    <div className="w-8 h-12 bg-amber-900/50 rounded border-2 border-amber-600/50 flex items-center justify-center">
      <span
        className="text-amber-200 text-2xl font-mono font-bold"
