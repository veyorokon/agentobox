...
**Note:** The Task tool has two modes (see FOUNDATIONS.md, Experiment 9):
- **Subagent mode** (no `team_name`): short-lived, blocks, events have `parent_tool_use_id`. These generate subagent transcript sidechains.
- **Teammate mode** (with `team_name` + `name`): long-lived, non-blocking, spawns a separate Claude process with its own session and own stream-json relay. Teammate events flow through the teammate's own relay — NOT as child events of the parent's Task call. Each teammate container has its own stream → backend pipeline.
...
```
CLAUDECODE=1
CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1   # May no longer be required (teams work without it in v2.1.42+), kept for compatibility
