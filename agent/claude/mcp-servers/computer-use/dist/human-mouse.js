 ...
 */
import { dist, sub, add, mult, normalize, clamp, cubicBezier, bezierControlPoints, } from './math.js';
const DEFAULTS = {
    targetWidth: 10,
    minSegmentPx: 3,
    segmentDecay: 0.6,
 ...
 * T = a + b * log2(D/W + 1)
 */
function fittsTime(distance, targetWidth) {
    const a = 50;
    const b = 150;
 ...
 */
function velocityEnvelope(t, aggression) {
 ...
 */
function computeSegments(from, to, decay, minPx, overshoot) {
 ...
 */
function segmentWaypoints(from, to, durationMs, aggression, wobbleAmplitude) {
 ...
 */
export function generateHumanPath(from, to, options) {
    ...
    const segments = computeSegments(from, to, opts.segmentDecay, opts.minSegmentPx, shouldOvershoot);
