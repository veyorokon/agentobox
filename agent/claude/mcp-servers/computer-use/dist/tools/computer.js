import { z } from 'zod';
import { mouse, keyboard, Point, screen, Button, imageToJimp, } from '@nut-tree-fork/nut-js';
 ...
 */
async function humanMove(to) {
    const from = await mouse.getPosition();
    const path = generateHumanPath({ x: from.x, y: from.y }, { x: to[0], y: to[1] });
 ...
 */
export async function humanScroll(direction, amount) {
    const scrollFn = direction === 'up'
        ? (px) => mouse.scrollUp(px)
        : (px) => mouse.scrollDown(px);
 ...
 */
export async function getApiToLogicalScale() {
...
export function registerComputer(server) {
        ...
        switch (action) {
            ...
            case 'get_screenshot': {
                ...
                if (process.platform === 'darwin') {
                    ...
                    if (apiScaleFactor < 1) {
                        image.resize(Math.floor(image.getWidth() * apiScaleFactor), Math.floor(image.getHeight() * apiScaleFactor));
                    }
